...
void View_LCD::print_lcd_XX(short x,const short y,const std::string& strIn, const bool transparency, unsigned short width, const unsigned char color,const Font& m_Font)
{
    ...
    for(unsigned short pos_str=0; pos_str<str1.length(); pos_str++) //проходим по всем символам строки
    {
        ...
        //Не >= т.к. это ширина, а не координата
        if ((x+symbol_width)>width) break;
...
void View_LCD::print_lcd(const Fnt _font_height,const short x,const short y,const std::string& strIn, const bool transparency, unsigned short width, const unsigned char _color)
{
    Font* cur_font=Lib_Fonts::Instance().get_Font(_font_height);
    if (cur_font)
    {
        print_lcd_XX(x,y,strIn,transparency,width,_color,*cur_font);
    }
    else
    {
